            ...
            <div class="relative flex flex-col">
                ...
                <div class="flex items-center px-5 space-x-2">
                    <a href="/"
                        ...
                        {{-- add auth()->user()->profile('logo'); if not exists add default Text logo --}}
                        @php
                            $userLogo = auth()->user()->profile('logo');
                            $organizationName = auth()->user()->profile('organization');
                        @endphp

                        @if ($userLogo)
                            <img src="{{ Storage::url($userLogo) }}" alt="{{ auth()->user()->name }}'s Logo"
                                class="w-auto h-7  ">
                        @else
                            <span class="text-xl font-bold text-zinc-900 dark:text-white">{{ $organizationName }}</span>
                        @endif
                ...
                <div
                    ...
                    <x-app.sidebar-link icon="phosphor-cube" href="{{ route('settings.subscription') }}"
                    ...
                    <x-app.sidebar-link
                        onclick="event.preventDefault(); new FilamentNotification().title('To be done').send()"
                        icon="phosphor-cube" :active="Request::is('plans')">Send Message</x-app.sidebar-link>
                    <x-app.sidebar-link
                        onclick="event.preventDefault(); new FilamentNotification().title('To be done').send()"
                        icon="phosphor-cube" :active="Request::is('plans')">Schedules</x-app.sidebar-link>
                    <x-app.sidebar-link
                        onclick="event.preventDefault(); new FilamentNotification().title('To be done').send()"
                        icon="phosphor-cube" :active="Request::is('plans')">Background Checks</x-app.sidebar-link>
                    <x-app.sidebar-link
                        onclick="event.preventDefault(); new FilamentNotification().title('To be done').send()"
                        icon="phosphor-cube" :active="Request::is('plans')">Updates</x-app.sidebar-link>
                    <x-app.sidebar-link href="{{ route('settings.profile') }}" icon="phosphor-cube"
                        :active="Request::is('settings/profile')">Settings</x-app.sidebar-link>
                    <x-app.sidebar-link
                        onclick="event.preventDefault(); new FilamentNotification().title('To be done').send()"
                        icon="phosphor-cube" :active="Request::is('plans')">Help</x-app.sidebar-link>

                    {{-- <x-app.sidebar-dropdown text="Projects" icon="phosphor-stack" id="projects_dropdown"
                        :active="Request::is('projects')" :open="Request::is('project_a') || Request::is('project_b') || Request::is('project_c')
                            ? '1'
                            : '0'">
                        <x-app.sidebar-link
                            onclick="event.preventDefault(); new FilamentNotification().title('Modify this button inside of sidebar.blade.php').send()"
                            icon="phosphor-cube" :active="Request::is('project_a')">Project A</x-app.sidebar-link>
                        <x-app.sidebar-link
                            onclick="event.preventDefault(); new FilamentNotification().title('Modify this button inside of sidebar.blade.php').send()"
                            icon="phosphor-cube" :active="Request::is('project_b')">Project B</x-app.sidebar-link>
                        <x-app.sidebar-link
                            onclick="event.preventDefault(); new FilamentNotification().title('Modify this button inside of sidebar.blade.php').send()"
                            icon="phosphor-cube" :active="Request::is('project_c')">Project C</x-app.sidebar-link>
            ...
            <div class="relative px-2.5 space-y-1.5 text-zinc-700 dark:text-zinc-400">
                ...
                <div x-show="sidebarTip" x-data="{ sidebarTip: $persist(true) }" class="px-1 py-3" x-collapse x-cloak>
                    <div
                        ...
                        <h5 class="pb-1 text-sm font-bold -translate-y-0.5">Notification</h5>
